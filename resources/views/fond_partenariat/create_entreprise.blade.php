      ...
      <div class="block">
        ...
        <div class="row">
            ...
            <div class="col-sm-12">
                ...
                <div class="step-app">
                <form id="progress-wizard" action="{{ route("entreprise.store") }}" method="post" enctype="multipart/form-data" class="form-horizontal">
                    @csrf
        ...
        <div id="progress-second" class="step">
                        <div class="row">
                                <fieldset>
                                    ...
                                    <div class="form-group">
                                        <label class=" control-label" for="">Quels sont les difficultés auxquelles vous êtes confrontés  <span data-toggle="tooltip" title="Quels sont les difficultés auxquelles vous êtes confrontés et qui justifie votre demande de financement ? "><i class="fa fa-info-circle"></i></span> </label>
                                            @foreach ($difficultes as $difficulte)
                                            <div class="col-lg-8 checkbox">
                                                <label><input type="checkbox" name='difficultes[]' value="{{ $difficulte->id }}"> {{ $difficulte->libelle }}</label>
                                            </div>
                                            @endforeach
                                        </select>
                                    </div>
                                    {{-- <div class="form-group">
                                        <label class="control-label " for="">Quels sont les difficultés auxquelles vous êtes confrontés <span class="text-danger">*</span></label>
                                        <select id="nb_annee_existences" name="nb_annee_existence" class="select-select2" data-placeholder="Choisir le maillon d'activite" style="width: 100%;" required onchange="afficherautre('maillon_activite',  {{ env('VALEUR_ID_AUTRE_MAILLON_ACTIVITE') }} ,'autre_maillon_activite');">
                                            <option></option><!-- Required for data-placeholder attribute to work with Chosen plugin -->
                                            @foreach ($nb_annee_existences as $nb_annee_existence )
                                                <option value="{{ $nb_annee_existence->id  }}" {{ old('nb_annee_existence') == $nb_annee_existence->id ? 'selected' : '' }} value="{{ $nb_annee_existence->id }}">{{ $nb_annee_existence->libelle }}</option>
                                    ...
                                    <div class="form-group">
                                        <label class=" control-label" for="example-chosen">Disposez-vous d'un systeme suivi de votre activité ?<span class="text-danger">*</span><span data-toggle="tooltip" title="Votre entreprise dispose t-elle d'un système de suivi financière et comptables "><i class="fa fa-info-circle"></i></span></label>
